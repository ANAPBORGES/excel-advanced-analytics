...
> A **working Excel workbook** on 57,542 real order lines, built with the formula patterns that carry actual analytical weight — `SUMPRODUCT`, `INDEX`/`MATCH`, `LARGE`, `PERCENTILE.INC`, structured references, two-way lookups — plus a **Power Query (M)** loader. Every number it produces was reconciled against BigQuery.
...
[![Validated](https://img.shields.io/badge/Numbers-validated%20vs%20BigQuery-4285F4?style=flat&logo=google-cloud&logoColor=white)](./docs/verified_numbers.md)
...
Full breakdown, with the BigQuery reconciliation, in [`docs/verified_numbers.md`](./docs/verified_numbers.md).
...
| [`docs/verified_numbers.md`](./docs/verified_numbers.md) | Excel vs BigQuery, metric by metric — plus data quality findings |
| [`docs/modern_excel_equivalents.md`](./docs/modern_excel_equivalents.md) | The same problems in Microsoft 365 (`XLOOKUP`, `LET`, dynamic arrays) |
...
| [`sql/02_control_totals.sql`](./sql/02_control_totals.sql) | The control totals every workbook figure is checked against |

---

## Techniques demonstrated

**Counting distinct orders in O(n).** Excel has no `COUNTDISTINCT`, and the usual `SUMPRODUCT(1/COUNTIF(range,range))` workaround is O(n²) — on 57,542 rows that is ~3.3 billion comparisons and the file hangs. Because the extract is sorted by `order_id` upstream, `=IF($A2<>$A1,1,0)` summed down the column gives 39,743, matching `COUNT(DISTINCT order_id)` exactly.
...
## Two engineering constraints, stated openly

**Excel 2016 compatibility.** The workbook uses no `XLOOKUP`, no `LET`, no dynamic arrays. That is deliberate: functions added after Excel 2007 need an `_xlfn.` prefix when a file is generated programmatically, and if the target Excel does not implement the function, the prefix survives into the saved file and every dependent cell shows `#NAME?`. The first build of this workbook used `XLOOKUP`, `UNIQUE`, `SORT`, `LET` and `TEXTJOIN` — all five broke and were rewritten. A portfolio file that opens with errors on the reviewer's machine has failed before anyone reads a formula. [`docs/modern_excel_equivalents.md`](./docs/modern_excel_equivalents.md) shows the modern form of each pattern.

**Excel and BigQuery differ by US$5.82 on US$1.78M profit — 0.0003%.** The export rounds cost and profit per line item; the warehouse does not. It is recorded in [`docs/verified_numbers.md`](./docs/verified_numbers.md) rather than quietly ignored, because "the spreadsheet and the warehouse disagree by a few units" is exactly what destroys trust in a report when someone else finds it first.
...
1. **Country names are not canonical** — the source carries both `Germany` and `Deutschland`, and `Brasil` rather than `Brazil`. A raw `GROUP BY` splits Germany across two rows and under-reports it. The workbook resolves it with an `INDEX`/`MATCH` mapping table on the `Lookups` sheet: consolidated, Germany is US$139,844.20.
2. **Cancelled and Returned items are present** in `order_items` and are excluded at the source query.
3. **No item in this window sells at a loss** — 0 of 57,542. That is a property of the generated dataset, not a business finding, and it is stated so nobody reads the 100% "sold at a profit" tile as a real insight. Loss-making segment analysis lives in the [Tableau project](https://github.com/ANAPBORGES/tableau-sales-profitability-dashboard), where 10 of 49 states genuinely lose money.
...
4. Verify against [`sql/02_control_totals.sql`](./sql/02_control_totals.sql).
